...
exports.createPages = async ({ actions }) => {
  ...
  if (fs.existsSync(examplesDataPath)) {
    ...
    chapters.forEach(chapter => {
      ...
      chapter.examples.forEach((example, index) => {
        ...
        const nextExample =
          index < chapter.examples.length - 1
            ? chapter.examples[index + 1]
            : null
    ...
    console.log(
      `✅ Created ${chapters.length} chapter pages and ${chapters.reduce(
    ...
    console.warn(
      "⚠️  examples.json not found. Run `npm run examples:ingest` first."
    )
  }
  ...
  if (fs.existsSync(stdlibDataPath)) {
    ...
    console.warn(
      "⚠️  stdlib.json not found. Run `npm run stdlib:ingest` first."
    )
  }
}
    ...
    externals:
      stage === "build-html" || stage === "develop-html"
        ? {
            ...
            "../../../videos/src/components/AnimatedFuchsiaSquare":
              "commonjs ../../../videos/src/components/AnimatedFuchsiaSquare",
          }
        : {},
    plugins: [
