...
const getAwayProgress = (timeMs, periods, transitionMs) => {
  ...
  for (const p of periods) {
    ...
    if (timeMs >= start && timeMs < start + transitionMs) {
      progress = Math.max(
        progress,
        easeInOutCubic(clamp01((timeMs - start) / transitionMs))
      )
      continue
    }
    ...
    if (timeMs >= end && timeMs < end + transitionMs) {
      progress = Math.max(
        progress,
        1 - easeInOutCubic(clamp01((timeMs - end) / transitionMs))
      )
      continue
    }
  }
...
const QueueVisual = ({ x, yTop, yBottom, isOverloaded, t, pulse = 0 }) => (
  <g>
    <path
      d={`M ${x - 25},${yTop - 10} L ${x - 25},${yBottom + 20}`}
      stroke={isOverloaded ? t.primary : t.surface2}
      strokeWidth={4}
      fill="none"
      strokeLinecap="round"
    />
...
const generateRawItems = config => {
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    if (supervisionMean) {
      ...
      const supervisionDur = Math.max(
        200,
        gaussian(rng, supervisionMean, supervisionStdDev)
      )
      ...
      if (isReturned) {
        ...
        steps.push({
          type: "orbit",
          startAng: 2.5 * Math.PI,
          targetAng: 1.5 * Math.PI,
          duration: retOrbitDur,
        })
...
const scheduleItems = (rawItems, config) => {
  ...
  const interval =
    inputIntervalMs ??
    Math.max(50, CYCLE_DURATION / Math.max(1, rawItems.length))
  ...
  if (stepBackAfterItems !== undefined && maxInFlight === 1) {
    ...
    for (const raw of supervisedItems) {
      const supervisionStep = raw.intrinsicSteps.find(
        s => s.type === "supervision"
      )
  ...
  for (const raw of sorted) {
    ...
    scheduled.push({
      ...
      endTime:
        scheduledSteps[scheduledSteps.length - 1]?.endTime ?? currentTime,
      tInputAdmission: scheduledSteps.find(s => s.type === "travel_to_input")
        ?.startTime,
      tInputQueueEntry: scheduledSteps.find(s => s.type === "input_queue")
        ?.startTime,
      tInputQueueExit: scheduledSteps.find(s => s.type === "input_queue")
        ?.endTime,
...
const HumanInTheLoopDiagram = ({
  ...
  const vFromQueueStart = {
    x: pHumanQueueBottom.x - 12,
    y: pHumanQueueBottom.y,
  }
  ...
  const vInputToAgentStart = {
    x: pInputQueueBottom.x + 8,
    y: pInputQueueBottom.y + 6,
  }
  ...
  const { items: rawItems, outageStart: effectiveOutageStart } =
    React.useMemo(() => {
      ...
      scenario,
      config.itemCount,
      config.autoProcessRate,
      config.returnToAgentRate,
      config.queueTime,
      config.agentProcessingTime,
      config.minOrbitTime,
      config.seed,
      config.outageStart,
      config.outageDuration,
      config.stepBackAfterItems,
      config.maxInFlight,
      config.supervisionMean,
      config.supervisionStdDev,
      config.useRampingSpawn,
      config.spawnJitterMs,
  ...
  const isInputOverloaded =
    itemsInInputQueue.length >= (mergedConfig.maxInputQueueCapacity ?? 6)
  ...
  if (stepBackAfterItems !== undefined) {
    ...
    const supervisedItems = capacityConstrainedItems
      .filter(item => item.flowType === "supervised")
      .sort((a, b) => {
        const aSupervisionStep = a.steps.find(s => s.type === "supervision")
        const bSupervisionStep = b.steps.find(s => s.type === "supervision")
        return (
          (aSupervisionStep?.startTime ?? 0) -
          (bSupervisionStep?.startTime ?? 0)
        )
      })
    ...
    const manualItems = capacityConstrainedItems
      .filter(
        item => item.flowType === "manual" && item.tQueueEntry !== undefined
      )
    ...
    if (manualItems.length > 0) {
      ...
      for (const item of manualItems) {
        ...
        while (
          ...
          manualHumanBusyUntil <= timeMs
        ) {
          if (manualCurrentlyProcessingItem) {
            processed.set(
              manualCurrentlyProcessingItem.id,
              manualHumanBusyUntil
            )
            ...
            if (
              manualSessionProcessedCount >= stepBackAfterItems &&
              manualWaitingQueue.length === 0
            ) {
        ...
        if (!manualHumanPresent && manualWaitingQueue.length > 5) {
          ...
          if (manualAwayStart !== null) {
            manualAbsencePeriods.push({
              start: manualAwayStart,
              end: arrivalTime,
            })
    ...
    itemsInHumanQueue.forEach((item, index) =>
      humanQueueMap.set(item.id, index)
    )
    ...
    itemsInHumanQueue = capacityConstrainedItems.filter(
      item =>
    ...
    if (finalOutageStart !== undefined && finalOutageStart !== null) {
      ...
      for (const idx of [
        ...
        if (endAbs <= cycleEnd) {
          ...
          periods.push({
            start: cycleBase,
            end: cycleBase + (endAbs % CYCLE_DURATION),
          })
        }
      }
  ...
  const humanAwayProgress = getAwayProgress(
    timeMs,
    humanAbsencePeriods,
    HUMAN_TRANSITION_MS
  )
  ...
  const renderItems = capacityConstrainedItems
    .map(item => {
      ...
      if (stepBackAfterItems !== undefined && item.flowType === "manual") {
        ...
        const supervisionIndex = item.steps.findIndex(
          s => s.type === "supervision"
        )
        ...
        if (completionTime !== undefined && supervisionIndex >= 0) {
          ...
          const totalPostSupervisionDuration = stepsAfterSupervision.reduce(
      ...
      let activeStep = item.steps.find(
      ...
      const effectiveP = Math.min(
        1,
        Math.max(0, (timeMs - activeStep.startTime) / activeStep.duration)
      )
      ...
      switch (effectiveStep.type) {
        ...
        case "travel_to_input":
          ...
          const pt0 = getQuadBezierPoint(
            p,
            pSpawnEnd,
            { x: inputQueueX, y: pInputQueueTop.y - 40 },
            pInputQueueTop
          )
          x = pt0.x
          y = pt0.y
        ...
        case "input_queue":
          x = pInputQueueTop.x
          {
            ...
            const dropP = easeInOutCubic(
              clamp01((timeMs - entryTime) / QUEUE_DROP_MS)
            )
        ...
        case "travel_to_agent":
          // Input Queue Bottom -> Agent Left
          const ptAgent = getQuadBezierPoint(
            p,
            pInputQueueBottom,
            cInputToAgent,
            pAgentLeft
        ...
        case "exit":
          ...
          const ptExitHuman = getQuadBezierPoint(
            p,
            pHumanQueueBottom,
            cHumanExit,
            pExitHuman
        ...
        case "exit_final":
          ...
          const ptExitFinal = getQuadBezierPoint(
            p,
            pAgentTop,
            cAgentExit,
            pExitFinal
    ...
    >
      ...
      {showMonkey && (
        <image
          ...
          opacity={1}
          style={{
            transition: disableCssTransitions
              ? undefined
      ...
      <QueueVisual
        x={inputQueueX}
        yTop={queueTopY}
        yBottom={queueBottomY}
        isOverloaded={isInputOverloaded}
        t={t}
        pulse={overloadPulse}
      />
      ...
      {!isAutonomous && !isCloselySupervised && (
        <QueueVisual
          ...
          yTop={queueTopY}
          yBottom={queueBottomY}
          isOverloaded={isHumanOverloaded}
          t={t}
          pulse={overloadPulse}
        />
      )}
      ...
      {/* Path: Input -> Agent */}
      <path
        d={`M ${vInputToAgentStart.x},${vInputToAgentStart.y} Q ${cInputToAgent.x},${cInputToAgent.y} ${vInputToAgentEnd.x},${vInputToAgentEnd.y}`}
        fill="none"
        stroke={t.primary}
        strokeWidth={2}
        strokeDasharray="4 4"
        markerEnd="url(#hitlArrow)"
        style={{
          transition: disableCssTransitions ? undefined : "opacity 0.3s",
          color: t.primary,
        }}
        opacity={showInputPath ? activePathOpacity : 0}
      />

      {/* Path: Agent -> Human Queue */}
      {!isAutonomous && (
        <path
          d={`M ${vToQueueStart.x},${vToQueueStart.y} Q ${cToHumanQueue.x},${cToHumanQueue.y} ${vToQueueEnd.x},${vToQueueEnd.y}`}
          fill="none"
          stroke={t.primary}
          strokeWidth={2}
          strokeDasharray="4 4"
          ...
          opacity={showToPath ? activePathOpacity : 0}
        />
      )}
      ...
      {!isAutonomous && (
        <path
          d={`M ${vFromQueueStart.x},${vFromQueueStart.y} Q ${cFromHumanQueue.x},${cFromHumanQueue.y} ${vFromQueueEnd.x},${vFromQueueEnd.y}`}
          fill="none"
          stroke={t.primary}
          strokeWidth={2}
          strokeDasharray="4 4"
          ...
          opacity={showFromPath ? activePathOpacity : 0}
        />
      )}

      {/* Path: Agent -> Exit (Over Human Queue) */}
      <path
        d={`M ${vAgentExitStart.x},${vAgentExitStart.y} Q ${cAgentExit.x},${cAgentExit.y} ${vAgentExitEnd.x},${vAgentExitEnd.y}`}
        fill="none"
        stroke={t.primary}
        strokeWidth={2}
        strokeDasharray="4 4"
        ...
        opacity={showAgentExitPath ? activePathOpacity : 0}
      />

      {/* Path: Human -> Exit */}
      {!isAutonomous && (
        <path
          d={`M ${vHumanExitStart.x},${vHumanExitStart.y} Q ${cHumanExit.x},${cHumanExit.y} ${vHumanExitEnd.x},${vHumanExitEnd.y}`}
          fill="none"
          stroke={t.primary}
          strokeWidth={2}
          strokeDasharray="4 4"
          ...
          opacity={showHumanExitPath ? activePathOpacity : 0}
        />
      )}
