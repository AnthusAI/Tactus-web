...
const appear = (progress, start, duration = 0.1) => {
  const t = clamp01((progress - start) / duration)
  return smoothstep(t)
}

const OldWayFlowchartDiagram = ({
  theme = "light",
  progress = 1,
  style,
  className,
  ...
  const edgeDraw = id => {
    ...
    const d = appear(p, item?.at ?? 0, 0.1)
    return d >= 0.999 ? 1 : d
  }
    ...
    >
      ...
      <g className="nodes">
        {nodes.map(node => {
          ...
          const transform = `translate(${node.x}, ${
            node.y
          }) scale(${s}) translate(${-node.x}, ${-node.y})`
          ...
          return (
            <polygon
              key={node.id}
              points={`${node.x},${node.y - half} ${node.x + half},${node.y} ${
                node.x
              },${node.y + half} ${node.x - half},${node.y}`}
              fill={t.primary}
              opacity={opacity}
              transform={transform}
            />
