...
const VisualEvaluationsDiagram = ({
  ...
  const renderItems = ITEMS.map(item => {
    ...
    let x = 0, y = 0, opacity = 1, status = "neutral"
  ...
  const completedItems = ITEMS.filter(item => localTime > item.spawnOffset + 1000)
  ...
  const score = totalCount === 0 ? 0 : Math.round((passCount / totalCount) * 100)
  ...
  return (
    ...
    >
      {/* Background Track */}
      <line x1={startX} y1={centerY} x2={endX} y2={centerY} stroke={t.surface2} strokeWidth={2} strokeDasharray="4 4" />

      {/* Nodes */}
      <g transform={`translate(${pStart.x}, ${pStart.y})`}>
        <circle r={20} fill={t.surface} stroke={t.surface2} strokeWidth={2} />
        <text y={36} textAnchor="middle" fontSize="10" fill={t.muted} fontFamily="sans-serif">Dataset</text>
      </g>

      <g transform={`translate(${pAgent.x}, ${pAgent.y})`}>
        <circle r={30} fill={t.surface} stroke={t.primary} strokeWidth={2} strokeOpacity={0.5 + agentPulse * 0.5} />
        <g transform="translate(-15, -15)">
          <Brain size={30} color={t.primary} />
        </g>
        <text y={46} textAnchor="middle" fontSize="10" fill={t.primary} fontFamily="sans-serif">Agent</text>
      </g>

      {/* Scoreboard Panel - Taller to fit gauge */}
      <g transform={`translate(${scoreboardX}, ${centerY})`}>
        <rect x={-50} y={-60} width={100} height={140} rx={8} fill={t.surface} stroke={t.surface2} strokeWidth={1} />
        
        {/* Title */}
        <text x={0} y={-40} textAnchor="middle" fontSize="10" fill={t.muted} fontFamily="sans-serif" fontWeight="600">RELIABILITY</text>
        
        {/* Score - Moved up */}
        <text x={0} y={-5} textAnchor="middle" fontSize="32" fill={t.ink} fontFamily="monospace" fontWeight="bold">
          {score}%
        </text>

        {/* Counts - Moved up */}
        <text x={0} y={15} textAnchor="middle" fontSize="10" fill={t.muted} fontFamily="sans-serif">
        ...
        <g transform="translate(0, 55)">
          {/* Gauge Arc */}
          <path d="M -30 0 A 30 30 0 0 1 30 0" fill="none" stroke={t.surface2} strokeWidth={4} />
          {/* Needle */}
          <g transform={`rotate(${gaugeAngle})`}>
            <line x1={0} y1={0} x2={0} y2={-26} stroke={t.primary} strokeWidth={2} strokeLinecap="round" />
      ...
      {renderItems.map(item => {
        ...
        return (
          <g key={item.id} transform={`translate(${item.x}, ${item.y})`} opacity={item.opacity}>
