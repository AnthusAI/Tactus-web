...
const VisualValidationDiagram = ({
  ...
  const renderItems = ITEMS.map(item => {
    ...
    let x = 0, y = 0, opacity = 1, scale = 1, color = t.ink
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${filterX}, ${centerY})`}>
        {/* Gate Graphic */}
        <rect x={-20} y={-40} width={40} height={80} rx={4} fill={t.surface} stroke={t.primary} strokeWidth={2} />
        {/* Hole (Square) - Page background color, no outline */}
        <rect x={-10} y={-10} width={20} height={20} fill="var(--color-bg)" />
        
        {/* Label */}
        <text y={56} textAnchor="middle" fontSize="11" fill={t.primary} fontFamily="sans-serif">Schema Validator</text>
      </g>

      {/* Start Label */}
      <text x={startX} y={centerY + 40} textAnchor="middle" fontSize="11" fill={t.muted} fontFamily="sans-serif">Unknown Inputs</text>
      
      {/* End Label */}
      <text x={endX} y={centerY + 40} textAnchor="middle" fontSize="11" fill={t.muted} fontFamily="sans-serif">Validated Data</text>
      ...
      {renderItems.map(item => {
        ...
        return (
          <g key={item.id} transform={`translate(${item.x}, ${item.y}) scale(${item.scale})`} opacity={item.opacity}>
             <g transform="translate(-12, -12)">
               <Shape size={24} color={item.color} fill={t.surface} strokeWidth={2} />
             </g>
          </g>
        )
      })}
