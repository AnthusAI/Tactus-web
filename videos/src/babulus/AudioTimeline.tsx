...
import type {
  ...
  AudioTrack,
  GeneratedTimeline,
} from "@/babulus/audioTypes"

const resolveClipSrc = (
...
const volumeAt = (
  base: number,
  envelope: Array<{ atSec: number; volume: number }>,
  localSec: number
) => {
...
const AudioClipPlayer: React.FC<{
  ...
  const v = envelope.length
    ? volumeAt(baseVolume, envelope, localSec)
    : baseVolume
  return <Audio src={staticFile(src)} volume={v} />
}

export const AudioTimelineLayer: React.FC<{ timeline: GeneratedTimeline }> = ({
  timeline,
}) => {
      ...
      {tracks.flatMap((track: AudioTrack) =>
        track.clips.map(
          (clip: AudioClipFile | AudioClipSfx | AudioClipMusic) => {
            ...
            const envelopeKey = clip.volumeEnvelope
              ? JSON.stringify(clip.volumeEnvelope)
              : ""
            const remaining = Math.max(1, durationInFrames - from)
            const clipDuration = (() => {
              if (
                (clip.kind === "sfx" || clip.kind === "music") &&
                clip.chosen?.durationSec != null
              ) {
                return Math.max(
                  1,
                  secondsToFrames(clip.chosen.durationSec, fps)
                )
              }
