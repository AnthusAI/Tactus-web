    ...
    switch (scene.id) {
      ...
      case "defense_in_depth":
        return (
          <DefenseInDepthScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "defense_in_depth")}
          />
        )
      case "least_privilege":
        return (
          <LeastPrivilegeScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "least_privilege")}
          />
        )
      case "sandbox_broker":
  ...
  return (
    <Layout>
      ...
      <div style={{ marginTop: 34, width: 1500, opacity: anim2 }}>
        <Body size="lg">
          If agents can take real actions with side effects, guardrails stop
          being a nice-to-have. They become the price of admission.
        </Body>
        <Body>
          • Prompts are suggestions, not controls.
          <br />
          • Guardrails are enforceable boundaries.
          <br />• Start with a threat model: assets, entry points, trust
    ...
    <Layout>
      ...
      <div style={{ display: "flex", gap: 28, marginTop: 32, opacity: anim2 }}>
        <div style={{ width: 470 }}>
          ...
          <Body>
            Hard limits, checklists, and layered safety didn’t reduce autonomy —
            they made delegation safe at scale.
          </Body>
        </div>
        <div style={{ width: 470 }}>
          <Body size="lg" weight={700} style={{ marginBottom: 10 }}>
          ...
          <Body>
            Protocols and checklists aren’t an insult to expertise. They’re how
            experts avoid catastrophic misses under pressure.
          </Body>
        </div>
        <div style={{ width: 470 }}>
          <Body size="lg" weight={700} style={{ marginBottom: 10 }}>
          ...
          <Body>
            Delegation requires boundaries: budgets, approval gates, audits, and
            reviews. Constraints enable autonomy.
          </Body>
        </div>
      </div>
    </Layout>
...
const PromptCeilingScene: React.FC = () => {
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 45 }}>
      ...
      <Body
        size="sm"
        style={{ opacity: anim2, textAlign: "center", maxWidth: 1500 }}
      >
        Prompts can reduce misbehavior — but not with enough reliability to
        safely scale in production.
      </Body>
      <div
        style={{
          marginTop: 0,
...
const ManualAssemblyScene: React.FC = () => {
  ...
  return (
    <Layout>
      ...
      <div style={{ marginTop: 34, width: 1500, opacity: anim2 }}>
        <Body size="lg">
          Many teams already add guardrails in Python — schemas, validation,
          retries, approval gates, sandboxing, secrets hygiene.
        </Body>
        <Body>
          But it’s easy to miss a layer when you’re assembling everything by
          convention across mismatched systems.
        </Body>
        <Body>
          ...
          <br />• Secrets isolation to prevent credential theft
        </Body>
      </div>
    </Layout>
  )
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 62 }}>
      ...
      <Body style={{ opacity: anim2, textAlign: "center", maxWidth: 1500 }}>
        No single technique solves everything. Guardrails work as defense in
        depth — layers that each reduce a different class of risk.
      </Body>
...
const LeastPrivilegeScene: React.FC<{
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 62 }}>
      ...
      <Body style={{ opacity: anim2, textAlign: "center", maxWidth: 1500 }}>
        Tactus enforces least privilege across multiple dimensions—not just tool
        access, but holistic minimal capability.
      </Body>
...
const SandboxBrokerScene: React.FC = () => {
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 62 }}>
      ...
      <Body style={{ opacity: anim2, textAlign: "center", maxWidth: 1500 }}>
        Sandbox untrusted orchestration. Keep secrets out of the runtime. Broker
        privileged calls behind a narrow, auditable boundary.
      </Body>
