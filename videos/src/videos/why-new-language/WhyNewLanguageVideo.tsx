...
export const WhyNewLanguageVideo: React.FC<WhyNewLanguageVideoProps> = ({
  ...
  const getCueTtsStarts = (sceneId: string, cueId: string): number[] => {
    ...
    return cueItem.segments
      .filter(seg => seg.type === "tts")
      .map(seg => seg.startSec)
  }
    ...
    switch (scene.id) {
      case "title_card":
        return (
          <TitleCardScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "title")}
          />
        )
      case "paradigm":
        return (
          <ParadigmScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "paradigm_shift")}
          />
        )
      case "machine_code_era":
        return (
          <MachineCodeScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "history")}
          />
        )
      case "control_flow":
        return (
          <ControlFlowScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "shift")}
          />
        )
      case "tools_strain":
        return (
          <ToolsStrainScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "python_problem")}
          />
        )
      case "practices_collapse":
        return (
          <PracticesCollapseScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "testing_breaks")}
          />
        )
      case "beyond_mlops":
        return (
          <BeyondMLOpsScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "mlops_comparison")}
          />
        )
      case "specifications":
        return (
          <SpecificationsScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "specs_and_evals")}
          />
        )
      case "props":
        return (
          <PrOpsScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "procedures")}
          />
        )
      case "new_language":
        return (
          <NewLanguageScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "first_class_primitives")}
          />
        )
      case "evolution":
        return (
          <EvolutionScene
            scene={scene}
            ttsStartsSec={getCueTtsStarts(scene.id, "conclusion")}
          />
        )
      case "cta":
        return <CTAScene />
...
const MachineCodeScene: React.FC<{ scene: Scene; ttsStartsSec: number[] }> = ({
  scene,
  ttsStartsSec,
}) => {
  const frame = useCurrentFrame()
  ...
  const titleSwap12 = interpolate(
    localSec,
    [Math.max(0, beat2 - 0.6), beat2 + 0.6],
    [0, 1],
    {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
  ...
  const titleSwap23 = interpolate(
    localSec,
    [Math.max(0, beat6 - 0.6), beat6 + 0.6],
    [0, 1],
    {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
...
const ControlFlowScene: React.FC<{ scene: Scene; ttsStartsSec: number[] }> = ({
  scene,
  ttsStartsSec,
}) => {
  const frame = useCurrentFrame()
...
const ToolsStrainScene: React.FC<{ scene: Scene; ttsStartsSec: number[] }> = ({
  scene,
  ttsStartsSec,
}) => {
  const frame = useCurrentFrame()
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 100 }}>
      ...
      >
        ...
        >
          <Body style={{ fontSize: 24, fontWeight: 900, marginBottom: 12 }}>
            Python
          </Body>
          <Code
            style={{ fontSize: 24, lineHeight: 1.3, whiteSpace: "pre-wrap" }}
          >
            {PYTHON_CODE}
          </Code>
        </Card>
...
const PracticesCollapseScene: React.FC<{
  scene: Scene
  ttsStartsSec: number[]
}> = () => {
  const frame = useCurrentFrame()
        ...
        {practices.map((practice, i) => {
          ...
          return (
            ...
            >
              ...
              <Body
                size="lg"
                style={{ fontSize: 28, flex: 1, lineHeight: 1.35 }}
              >
                {practice}
              </Body>
            </Card>
...
const BeyondMLOpsScene: React.FC<{ scene: Scene; ttsStartsSec: number[] }> = ({
  scene,
  ttsStartsSec,
}) => {
  const frame = useCurrentFrame()
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 120 }}>
      ...
      <div style={{ width: "100%", maxWidth: 1700, display: "flex", gap: 32 }}>
        ...
        >
          <Body style={{ fontSize: 32, fontWeight: 900, marginBottom: 24 }}>
            MLOps
          </Body>
...
const SpecificationsScene: React.FC<{
  scene: Scene
  ttsStartsSec: number[]
}> = () => {
  const frame = useCurrentFrame()
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 120 }}>
      ...
      >
        <Code style={{ fontSize: 28, lineHeight: 1.4, whiteSpace: "pre-wrap" }}>
          {specCode}
        </Code>
      </Card>
  ...
  return (
    <Layout justify="flex-start" style={{ paddingTop: 120 }}>
      ...
      >
        ...
        <br />
        <br />
        <span style={{ color: "#c7007e", fontSize: "1.2em" }}>
          PrOps operates procedures
        </span>
      </Body>
    </Layout>
  )
...
const NewLanguageScene: React.FC<{
  scene: Scene
  ttsStartsSec: number[]
}> = () => {
  const frame = useCurrentFrame()
          ...
          {features.map((feature, i) => {
            ...
            return (
              ...
              >
                <Body
                  size="lg"
                  style={{ fontSize: 32, fontWeight: 800, color: "#c7007e" }}
                >
                  {feature}
                </Body>
              </Card>
